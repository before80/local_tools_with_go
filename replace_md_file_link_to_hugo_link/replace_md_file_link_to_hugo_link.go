	...
	err = filepath.WalkDir(rootDir, func(path string, d fs.DirEntry, err error) error {
		...
		if strings.ToLower(filepath.Ext(fileName)) == ".md" {
			...
			if fileName == "_index.md" {
				noExtPath = strings.Replace("/"+strings.TrimPrefix(path, "./"), "_index.md", "", 1)
			} else {
				noExtPath = strings.Replace("/"+strings.TrimPrefix(path, "./"), ".md", "", 1)
			}
			noExtPath = filepath.ToSlash(noExtPath)
			noExtPath = strings.TrimSuffix(noExtPath, "/")

			v := fmt.Sprintf("{{< ref \"%s\" >}}", "/"+currentExecutableDirName+noExtPath)

			//fmt.Println(noExtPath)
